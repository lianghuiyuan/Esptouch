...
- (void) __listenAsyn: (const int) expectDataLen
{
    ...
    dispatch_async(queue, ^{
        ...
        if (DEBUG_ON)
        {
            ...
            NSLog(@"getExpectTaskResultCount = %d", [self._parameter getExpectTaskResultCount]);
        }
        ...
        while ([self._esptouchResultArray count] < [self._parameter getExpectTaskResultCount] && !self._isInterrupt)
        {
            ...
            if (DEBUG_ON)
            {
                NSLog(@"ESPTouchTask __listenAsyn() 监听接收到数据 receiveData(%lu): %@", (unsigned long)[receiveData length], [[NSString alloc] initWithData:receiveData encoding:NSUTF8StringEncoding]);
            }
            if (receiveData != nil)
            {
    ...
    while (!self._isInterrupt)
    {
        if (currentTime - lastTime >= [self._parameter getTimeoutTotalCodeMillisecond]/1000.0)
        {
            if (DEBUG_ON)
            {
                ...
                //NSLog(@"gc code=%@  TargetHostName=%@   port=%d",gcBytes2, [self._parameter getTargetHostname], [self._parameter getTargetPort]);
            }
            // send guide code
        ...
        else
        {
            if (DEBUG_ON)
            {
                //NSLog(@"dc code=%@  TargetHostName=%@   port=%d",gcBytes2, [self._parameter getTargetHostname], [self._parameter getTargetPort]);
            }
...
- (NSArray*) executeForResults:(int) expectTaskResultCount
{
    ...
    if(DEBUG_ON) {
        NSLog(@"executeForResults expectTaskResultCount=====%d", expectTaskResultCount);
    }
